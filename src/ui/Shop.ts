...
export class Shop {
  ...
  update(input: Input, inventory: Inventory): string[] {
    if (!this.open) return [];
    const msgs: string[] = [];
    ...
    if (input.anyPressed(["Enter", "Space", "KeyJ"])) {
      if (this.tab === "buy") this.buy(inventory, msgs);
  ...
  private buy(inventory: Inventory, msgs: string[]): void {
  ...
  private sell(inventory: Inventory, msgs: string[]): void {
  ...
  render(r: Renderer, inventory: Inventory): void {
    if (!this.open) return;
    const W = r.width;
    const H = r.height;
    const x = 80;
