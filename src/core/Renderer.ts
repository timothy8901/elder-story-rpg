...
export class Renderer {
  ...
  fillRectWorld(
    ...
    this.ctx.fillStyle = color;
    this.ctx.fillRect(
      Math.round(x - cam.renderX),
      Math.round(y - cam.renderY),
      Math.round(w),
      Math.round(h),
    );
  }
  ...
  strokeRectWorld(
    ...
    this.ctx.strokeRect(
      Math.round(x - cam.renderX) + 0.5,
      Math.round(y - cam.renderY) + 0.5,
      Math.round(w) - 1,
      Math.round(h) - 1,
    );
  }
   ...
   */
  vignette(tint: string | null, tintAlpha: number, focusX?: number, focusY?: number): void {
    const ctx = this.ctx;
  ...
  private roundPath(x: number, y: number, w: number, h: number, radius: number): void {
    const ctx = this.ctx;
