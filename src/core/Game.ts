...
export class Game {
  ...
  update(dt: number): void {
    ...
    if (this.shop.open) {
      ...
      if (msgs.length) {
        for (const m of msgs) this.hud.pushToast(m, "#ffe45e");
  ...
  private updateWorld(dt: number): void {
    ...
    if (this.input.justPressed("KeyE")) {
      ...
      if (npc) {
        if (npc.vendor) this.shop.start(npc.name);
  ...
  render(fps: number): void {
    ...
    r.withWorld(cam, (ctx) => {
      for (const e of this.combat.enemies) {
        if (this.isQuestTarget(e)) drawQuestArrow(ctx, e.body.centerX, e.body.pos.y, time);
      }
    });
  ...
  private npcMarker(npc: NPC): NpcMarker {
    if (npc.vendor) return "$";
    if (npc.faction) {
      const id = npc.faction;
      if (this.factions.isRival(id)) return "";
      const s = this.factions.get(id);
      if (!s.joined) return "?"; // a quest (membership) to offer
      if (s.stage === 2) return "!"; // objective met — turn it in
      return "";
    }
  ...
  private isQuestTarget(e: Enemy): boolean {
    if (e.isDragon) {
      const mq = this.mainQuest.stage;
      if (mq === "dragonRising" || mq === "bladeInDark" || mq === "dragonslayer") return true;
      const blades = this.factions.get("blades");
      if (blades.joined && blades.stage === 1) return true;
    }
