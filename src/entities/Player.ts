...
import { Entity, type UpdateContext } from "./Entity.js";
...
export class Player extends Entity {
  ...
  private applyHorizontal(dt: number, left: boolean, right: boolean, speedScale = 1): void {
    const b = this.body;
    ...
    if (left && !right) {
      ...
      if (Math.abs(b.vel.x) <= drop) b.vel.x = 0;
      else b.vel.x -= Math.sign(b.vel.x) * drop;
    }
  ...
  private isOnOneWay(map: Tilemap): boolean {
    if (!this.body.onGround) return false;
    const b = this.body;
    const row = map.rowAt(b.bottom + 1);
    const c0 = map.colAt(b.pos.x);
    const c1 = map.colAt(b.right - 1e-4);
    for (let col = c0; col <= c1; col++) {
      if (map.tileAt(col, row) === TileType.OneWay) return true;
    }
    return false;
  }
