...
export class Enemy {
  ...
  /** XP-loot level used when this enemy drops items. */
  lootLevel: number;
  ...
  constructor(x: number, y: number, opts?: { health?: number; damage?: number; kind?: string; lootLevel?: number; w?: number; h?: number }) {
  ...
  fear(seconds: number): void {
    this.fleeTimer = Math.max(this.fleeTimer, seconds);
  }

  update(dt: number, map: Tilemap, playerCenterX: number, _playerCenterY = 0): void {
    const b = this.body;
    ...
    if (b.onGround) {
      ...
      if (blocked || atLedge) this.dir = (-this.dir) as -1 | 1;
    }
  }
