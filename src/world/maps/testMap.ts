...
export function createTestMap(): GameMap {
  const grid: TileType[][] = Array.from({ length: ROWS }, () =>
    Array.from({ length: COLS }, () => TileType.Empty),
  );

  const set = (col: number, row: number, t: TileType): void => {
    if (row >= 0 && row < ROWS && col >= 0 && col < COLS) grid[row]![col] = t;
  };
  const fillRow = (row: number, c0: number, c1: number, t: TileType): void => {
    for (let c = c0; c <= c1; c++) set(c, row, t);
  };
  const fillCol = (col: number, r0: number, r1: number, t: TileType): void => {
    for (let r = r0; r <= r1; r++) set(col, r, t);
  };
