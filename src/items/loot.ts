...
/**
 * Modular loot generation. A base template defines the item's frame; random
 * prefixes scale its damage/armor, and suffixes attach an enchantment. Value is
 * recomputed from the final stats so rarer rolls are worth more.
 */
...
interface BaseTemplate {
  ...
  armor?: number;
  effect?: Item["effect"];
  /** Fixed enchantments baked into the base (e.g. faction reward gear). */
  enchantments?: { stat: EnchantStat; magnitude: number }[];
}
...
const SUFFIXES: { word: string; stat: EnchantStat; magnitude: number }[] = [
 ...
 */
export function generateLoot(level = 1): Item {
  ...
  const prefixes: Affix[] = [];
  const suffixes: typeof SUFFIXES = [];

  if (base.damage != null || base.armor != null) {
    if (chance(0.35 + level * 0.03)) prefixes.push(pick(PREFIXES));
  }
  if (canEnchant && chance(0.3 + level * 0.03)) suffixes.push(pick(SUFFIXES));
...
function buildItem(
  base: BaseTemplate,
  prefixes: Affix[],
  suffixes: { word: string; stat: EnchantStat; magnitude: number }[],
): Item {
  ...
  if (base.damage != null) item.damage = Math.round(base.damage * prefixMult);
  if (base.armor != null) item.armor = Math.round(base.armor * prefixMult);
  if (base.effect) item.effect = { ...base.effect };
  if (base.enchantments) item.enchantments.push(...base.enchantments.map((e) => ({ ...e })));
  if (base.type === ItemType.Consumable) {
    item.stackable = true;
    item.quantity = 1;
  }
  if (prefixes.length) item.prefix = prefixes[prefixes.length - 1]!.word;
  if (suffixes.length) {
    const s = suffixes[0]!;
    item.suffix = s.word;
  ...
  for (const e of item.enchantments) value += e.magnitude * 8;
